    ...
    private void Update()
    {
        if (gc.getActiveAction() == "camera" && !gc.GetGameOver() && !switchingToNewObject)
        {
            ...
            if (Input.touchCount == 0)
            {
                ...
                if (scroll != 0f)
                {
                    zoomSlider.GetComponent<Slider>().value = Mathf.Clamp(
                        Camera.main.orthographicSize - scroll * minZoom * 0.1f,
                        50f, minZoom);
                }
                if (Input.GetMouseButtonDown(0) && Input.mousePosition.x > num)
                {
                    mouseLastPosition = Input.mousePosition;
                    mousePanning = false;
                }

                if (Input.GetMouseButton(0) && Input.mousePosition.x > num)
                {
                    Vector2 mouseDelta = (Vector2)(Input.mousePosition - mouseLastPosition);
                    mouseLastPosition = Input.mousePosition;

                    if (mouseDelta.magnitude > 2f || mousePanning)
                    {
                        mousePanning = true;
                        panning = true;
                        PlanetSelect.panning = true;
                        ...
                        Vector3 worldZero = Camera.main.ScreenToWorldPoint(Vector2.zero);
                        Vector3 worldRight = Camera.main.ScreenToWorldPoint(Vector2.right);
                        float pixelToWorld = Vector2.Distance(worldZero, worldRight);

                        float dx = (0f - mouseDelta.x) * pixelToWorld * mouseSensitivity;
                        float dy = (0f - mouseDelta.y) * pixelToWorld * mouseSensitivity;

                        Vector2 newPos = new Vector2(
                            Mathf.Abs(base.transform.position.x + dx),
                            Mathf.Abs(base.transform.position.y + dy));

                        if (newPos.magnitude < playableRegionScale.x)
                        {
                            base.transform.Translate(dx, dy, 0f);
                        }
                        else if (base.transform.position.magnitude > playableRegionScale.x)
                        {
                            base.transform.position = base.transform.position.normalized * playableRegionScale.x;
