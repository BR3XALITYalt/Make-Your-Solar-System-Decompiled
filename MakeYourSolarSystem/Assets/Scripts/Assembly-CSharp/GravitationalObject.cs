...
public class GravitationalObject : MonoBehaviour
{
	...
	private void PlanetCollision(Collision2D otherPlanet)
	{
		...
		SetSelectBoxScale();
		if (gc.GetMapName() == "ModeTriadTimeAttack")
		{
			if ((base.name == "RedStarLeft" || base.name == "RedStarRight" || base.name == "RedStarUp") && (otherPlanet.gameObject.name == "RedStarLeft" || otherPlanet.gameObject.name == "RedStarRight" || otherPlanet.gameObject.name == "RedStarUp"))
			{
				gc.GetGoogleGames().UnlockAchievement("ThreesCrowd");
			}
		}
		else if (!gc.GetGameOver() && objectType == "blackHole" && mass2 > 42500f && mass2 <= 125000f)
		{
			gc.GetGoogleGames().UnlockAchievement("OrangeNewBlack");
		}
	...
	private void SetObjectSurface(float mass)
	{
		...
		if ((objectType == string.Empty || objectType == "brownDwarf") && mass >= (float)minFusionStarMass)
		{
			...
			objectType = "star";
			setSize();
			if (!gc.GetGameOver() && itWasPlanet)
			{
				gc.GetGoogleGames().UnlockAchievement("PlanetToStar");
			}
		}
		if (objectType == "star" && !pulsates && size >= 500f)
		{
			...
			pulsates = true;
			if (!gc.GetGameOver() && mass > 600000f && mass < (float)maxStarMass && !initialObject)
			{
				gc.GetGoogleGames().UnlockAchievement("ImBlue");
			}
		}
		if (objectType == "star" && mass >= (float)maxStarMass)
		{
			...
			if (!gc.GetGameOver() && gc.GetMode() != "supernova")
			{
				gc.GetGoogleGames().UnlockAchievement("Supernova");
			}
			if (gc.GetMapName() == "ModeBigTimeAttack")
			{
				gc.GetGoogleGames().UnlockAchievement("ScreenBreaker");
			}
		...
		if (objectType == "neutron" && mass >= blackHoleMinMass)
		{
			...
			setSize();
			if (!gc.GetGameOver() && !initialObject)
			{
				gc.GetGoogleGames().UnlockAchievement("BlackHole");
			}
		}
		starColor = getStarColor(mass);
		delayedColorUpdate();
