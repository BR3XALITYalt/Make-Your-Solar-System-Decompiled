...
public class GPGSCalls : MonoBehaviour
{
	...
	public void LogInGooglePlayGames()
	{
		...
		Social.localUser.Authenticate(delegate(bool success)
		{
			...
			{
				currentPlayerId = Social.localUser.id;
				currentPlayerName = Social.localUser.userName;
				loggedIntoGoogleGames = true;
			}
			gc.SetTryedToLogInGoogleGames(true);
		});
	}
	...
	public void UnlockIncrementalAchievement(string name)
	{
		...
		{
		case "ShareFiveTimes":
			empty = achievementShareFiveTimes;
			break;
		}
		((PlayGamesPlatform)Social.Active).IncrementAchievement(empty, 1, delegate
		{
		});
	}
	...
	{
		PlayGamesPlatform.Instance.LoadScores(GetLeaderboardIdFromMapName(mapName), LeaderboardStart.PlayerCentered, 1, LeaderboardCollection.Public, LeaderboardTimeSpan.AllTime, delegate(LeaderboardScoreData data)
		{
			prevScore = (int)data.PlayerScore.value;
			Social.ReportScore(score, GetLeaderboardIdFromMapName(mapName), delegate(bool success)
			{
				if (!success)
				{
					gc.setNoInternetOrLoggedIntoGoogleGames();
				}
				else
				{
					GetIngameLeaderboard(mapName);
				}
			});
		});
	}
	...
	public void GetIngameLeaderboard(string mapName)
	{
		...
		lb.id = GetLeaderboardIdFromMapName(mapName);
		lb.timeScope = TimeScope.AllTime;
		lb.LoadScores(delegate(bool ok)
		{
			if (ok)
			{
	...
	public void ShowSpecificLeaderboard(string mapName)
	{
		if (loggedIntoGoogleGames)
		{
			((PlayGamesPlatform)Social.Active).ShowLeaderboardUI(GetLeaderboardIdFromMapName(mapName));
		}
	}
	...
	{
		PlayGamesPlatform.Instance.Events.IncrementEvent(eventPlanetsLost, 1u);
	}
