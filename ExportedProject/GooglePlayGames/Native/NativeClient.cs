...
namespace GooglePlayGames.Native
{
	public class NativeClient : IPlayGamesClient
	{
		...
		public void GetIdToken(Action<string> idTokenCallback)
		{
			...
			mTokenClient.SetRationale(rationale);
			mTokenClient.GetIdToken(string.Empty, idTokenCallback);
		}
		...
		public void GetServerAuthCode(string serverClientId, Action<CommonStatusCodes, string> callback)
		{
			...
			{
				CommonStatusCodes commonStatusCodes = ConversionUtils.ConvertResponseStatusToCommonStatus(serverAuthCodeResponse.Status());
				if (commonStatusCodes != CommonStatusCodes.Success && commonStatusCodes != CommonStatusCodes.SuccessCached)
				{
					GooglePlayGames.OurUtils.Logger.e("Error loading server auth code: " + serverAuthCodeResponse.Status());
				}
				if (callback != null)
				{
					callback(commonStatusCodes, serverAuthCodeResponse.Code());
				}
			});
		}
		...
		public void GetPlayerStats(Action<CommonStatusCodes, GooglePlayGames.BasicApi.PlayerStats> callback)
		{
			...
			{
				CommonStatusCodes commonStatusCodes = ConversionUtils.ConvertResponseStatusToCommonStatus(playerStatsResponse.Status());
				if (commonStatusCodes != CommonStatusCodes.Success && commonStatusCodes != CommonStatusCodes.SuccessCached)
				{
				...
				if (callback != null)
				{
					...
					{
						callback(commonStatusCodes, playerStatsResponse.PlayerStats().AsPlayerStats());
					}
					else
					{
						callback(commonStatusCodes, new GooglePlayGames.BasicApi.PlayerStats());
					}
				}
			});
		}
